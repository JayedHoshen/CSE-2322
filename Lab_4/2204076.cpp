/*
    ...
    1) Implement single rotations (Left-Rotate and Right-Rotate) on a binary search tree (BST).
    2) After every rotation, correctly recompute and store the height of every node.
    3) Print each node’s key together with its updated height (use an in-order traversal).
*/
